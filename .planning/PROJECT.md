...
- [ ] User can select "FTX-1" as the active radio (alongside QMX/QDX/KH1) in station configuration
- [ ] Firmware establishes CAT control with the FTX-1 over its USB CP210x virtual COM port (via a new usb_host_vcp-based backend)
...
- Decision: bring in Espressif's `usb_host_vcp` component (official, maintained, supports CP210x/FTDI/CH34x as a unified virtual COM API) as a new dependency for FTX-1 CAT, rather than hand-rolling a CP210x vendor-request driver or requiring the CAT-3 wired UART fallback.
...
- **New dependency**: Espressif `usb_host_vcp` component required for CP210x CAT support — first non-CDC-ACM, non-UAC USB dependency in the project
...
|----------|-----------|---------|
| Use Espressif's `usb_host_vcp` component for CP210x CAT, not a custom driver or CAT-3 UART | Official/maintained component covers CP210x correctly; custom vendor-request driver is more maintenance burden; CAT-3 UART contradicts the USB-only requirement | — Pending |
